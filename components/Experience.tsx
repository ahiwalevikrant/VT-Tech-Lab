    ...
    <section id="experience" className="mx-auto max-w-content px-4 sm:px-6 lg:px-8 py-10 sm:py-16 md:py-24">
      ...
      <div className="mt-8 sm:mt-12 space-y-4 sm:space-y-6">
        {experience.map((role) => (
          <div
            ...
            className="glass-card p-5 sm:p-7 md:p-8 transition-all duration-300 hover:border-cyan-400/50"
          >
            <div className="grid gap-4 sm:gap-6 md:grid-cols-[240px_1fr] md:gap-10">
              <div>
                <span className="rounded-full bg-cyan-500/15 border border-cyan-500/30 px-2.5 sm:px-3 py-0.5 sm:py-1 font-mono text-[11px] sm:text-xs font-bold text-cyan-300">
                  {role.period}
                </span>
                <h3 className="mt-2.5 sm:mt-3 text-lg sm:text-xl font-bold text-white">{role.role}</h3>
                <p className="mt-0.5 sm:mt-1 font-sans text-xs sm:text-sm font-semibold text-purple-400">{role.company}</p>
              </div>
      ...
      <div className="glass-card mt-6 sm:mt-8 p-5 sm:p-7 md:p-8">
        <div className="grid gap-2 sm:gap-3 md:grid-cols-[240px_1fr] md:gap-10">
          <div>
            <span className="rounded-full bg-white/10 px-2.5 sm:px-3 py-0.5 font-mono text-[11px] sm:text-xs text-slate-300">
              {education.period}
            </span>
            <h3 className="mt-1.5 sm:mt-2 text-sm sm:text-base font-bold text-white">{education.degree}</h3>
          </div>
          <p className="text-xs sm:text-sm text-slate-300 self-center">
